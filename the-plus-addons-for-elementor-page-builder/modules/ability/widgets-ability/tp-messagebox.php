...
wp_register_ability('tpae/tpae-messagebox', [
    'label' => __('Message Box', 'tpebl'),
    'description' => __('Adds the The Plus "Message Box" widget (tp-messagebox) to an Elementor container.', 'tpebl'),
    'category' => 'tpae',
...
function tpae_mcp_add_theplus_messagebox_permission(?array $input = null): bool {
...
function tpae_mcp_add_theplus_messagebox_ability(array $input) {
    if (!tpae_mcp_has_elementor()) { return new WP_Error('elementor_missing', __('Elementor must be active.', 'tpebl')); }
    $widget_type = 'tp-messagebox';
    if (!tpae_mcp_has_registered_widget($widget_type)) { return new WP_Error('widget_missing', __('The Plus Message Box widget is not registered.', 'tpebl')); }
    $post_id = absint($input['post_id'] ?? 0);
